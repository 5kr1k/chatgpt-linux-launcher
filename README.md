...
Un mini-proyecto simple para integrar **ChatGPT** como una pseudo-aplicación
nativa en Linux, usando **Chromium** en modo `--app` y un lanzador `.desktop`.

Permite abrir ChatGPT en una ventana limpia, sin barra de direcciones ni
...
- Abre ChatGPT en una ventana minimalista tipo "app".
...
Guarda el archivo `chatgpt-app` en:
~/.local/bin/chatgpt-app

Y dale permisos:

```bash
chmod +x ~/.local/bin/chatgpt-app
``` 
---

### 2. Instalar el lanzador

Copia chatgpt.desktop en:
...
para que la ventana con clase ChatGPT se abra:
...
ChatGPT en su escritorio Linux de forma más limpia.
